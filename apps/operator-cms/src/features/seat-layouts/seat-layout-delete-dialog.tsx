...
export function SeatLayoutDeleteDialog({
  ...
  async function handleConfirm() {
    /* v8 ignore next -- dialog only opens when layout is non-null */
    if (!layout) return;
    try {
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel data-testid="seat-layout-delete-cancel">Cancel</AlertDialogCancel>
          <AlertDialogAction
            onClick={() => void handleConfirm()}
            className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            disabled={deleteMutation.isPending}
            data-testid="seat-layout-delete-confirm"
          >
